...
class OrderFeedLocators:
    ...
    ORDER_IN_PROGRESS = (By.XPATH, "//p[text()='В работе:']/following-sibling::ul/li[contains(@class, 'text_type_digits-default')]")

    @staticmethod
    def in_progress_order_locator(order_number):
        """Возвращает локатор для конкретного номера заказа в разделе 'В работе'."""
        return (By.XPATH,
                f"//p[text()='В работе:']/following-sibling::ul/li[contains(@class, 'text_type_digits-default') and text()='{order_number}']")
